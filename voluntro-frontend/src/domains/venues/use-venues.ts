...
import { ALL_VENUES, SINGLE_VENUE, VENUE_QUERY } from "#/shared/constants/query-keys.ts";
...
export function useUpdateVenue(venueId: string) {
  ...
  return useMutation({
    ...
    onError: (error: ApiError) => {
      toast.error("Unable to update venue", { description: error.responseBody });
    },
  });
}
...
export function useDeleteVenue(venueId: string) {
  ...
  return useMutation({
    ...
    onError: (error: ApiError) => {
      toast.error("Unable to delete venue", { description: error.responseBody });
    },
  });
}

async function venueComboboxQuery(query: string) {
  return await apiFetch<SelectOption[]>(`/query/venues?${query}`);
}
export function useVenueComboboxQuery(query: string) {
  return useQuery({
    queryKey: [ALL_VENUES, VENUE_QUERY, { query }],
    queryFn: () => venueComboboxQuery(query),
  });
}
