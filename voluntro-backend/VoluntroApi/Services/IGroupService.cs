...
public interface IGroupService
{
    ...
    /// </summary>
    Task<PagedResult<GroupSummary>> GetAllAsync(GetGroupsQuery query, CancellationToken cancellationToken, bool includeDeleted);

    /// <summary>
    /// Returns a single group with its direct children and active member count, or <c>null</c> if not found.
    /// </summary>
    Task<GroupDetails?> GetByIdAsync(Guid id, CancellationToken cancellationToken, bool includeDeleted);

    /// <summary>
    /// Creates a new group. Returns <c>null</c> if the specified parent group does not exist or is deleted.
    /// </summary>
    Task<GroupDetails?> CreateAsync(CreateGroupRequest request, CancellationToken cancellationToken);

    /// <summary>
    /// Updates an existing group. Returns <c>null</c> if not found, parent not found, or if the new parent would create a cycle.
    /// </summary>
    Task<(GroupDetails?, UpdateGroupResult)> UpdateAsync(Guid id, UpdateGroupRequest request, CancellationToken cancellationToken);
    ...
    /// </summary>
    Task<(GroupDetails?, RestoreGroupResult)> RestoreAsync(Guid id, CancellationToken cancellationToken);
}