...
public interface IVenueService
{
    ...
    /// </summary>
    Task<PagedResult<VenueSummary>> GetAllAsync(GetVenuesQuery query, CancellationToken cancellationToken);

    /// <summary>
    /// Returns a venue by its unique identifier, or <c>null</c> if not found.
    /// </summary>
    Task<VenueDetails?> GetByIdAsync(Guid venueId, CancellationToken cancellationToken, bool includeDeleted);

    /// <summary>
    /// Creates a new venue and returns the created record.
    /// </summary>
    Task<VenueDetails> CreateAsync(CreateVenueRequest request, CancellationToken cancellationToken);

    /// <summary>
    /// Updates an existing venue and returns the updated record, or <c>null</c> if not found.
    /// </summary>
    Task<VenueDetails?> UpdateAsync(Guid venueId, UpdateVenueRequest request, CancellationToken cancellationToken);
